...
x_train_error = []
x_test_error = []
for i in enumerate (range(2, 105)):
    estimators.append(i[1] * 10)
    xgbr = XGBRegressor(learning_rate=0.05, n_estimators=(i[1])*10, random_state=77, verbose=False )
...
# Based on above results will be choosing no of estimators as 850
...
for i in enumerate (range(1, 20)):
    ...
    xgbr = XGBRegressor(learning_rate=0.05, n_estimators=120, max_depth=(i[1])*1, random_state=77, verbose=False )
    xgbr.fit(X_train, y_train)
...
xgbr = XGBRegressor(learning_rate=0.05, n_estimators=650, max_depth=3, random_state=77, verbose=False )
