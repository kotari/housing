...
for i in enumerate(range(1, 21)):
    print(i[1])
    estimators.append(i[1] * 10)
    rfr = RandomForestRegressor(n_estimators=(i[1])*10, min_samples_split=10, bootstrap=True)
...
# Based on above results will be choosing no of estimators as 80
# Next choose max depth
estimators = []
x_train_error = []
x_test_error = []
for i in enumerate (range(1, 100)):
    print(i[1])
    estimators.append(i[1] * 1)
    rfr = RandomForestRegressor(n_estimators=70, max_depth=(i[1])*1, min_samples_split=10, bootstrap=True)
    rfr.fit(X_train, y_train)
...
rfr = RandomForestRegressor(n_estimators=70, max_depth=12, min_samples_split=10, bootstrap=True) #score 3182 on leadershipboard
...
feat_imp = zip(df_train.columns, rfr.feature_importances_)
...
predictions = rfr.predict(df_test_converted)
